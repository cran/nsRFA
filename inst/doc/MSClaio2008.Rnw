...
bitmap(file="serie.png", type="png256", height=6, width=8, res=144, pointsize=16)
@
<<fig=FALSE, width=5, height=5>>=
...
\end{center}
\caption{Series of maximum annual flood peaks in station 69023.}
\label{fig:serie}
\end{figure}
...
summary(MSC)
@
The candidate distributions and the selected ones can be plotted in a log-normal probability plot (Figure \ref{fig:plotMSC}):
<<echo=FALSE, eval=TRUE>>=
bitmap(file="plotMSC.png", type="png256", height=8, width=8, res=144, pointsize=16)
@
<<fig=FALSE, width=5, height=5>>=
plot(MSC)
@
...
\end{center}
\caption{Data (Weibull plotting position) and candidate distributions in lognormal probability plot.
