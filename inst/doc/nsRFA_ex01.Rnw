...
Some diagnostics can be plotted as (Figure \ref{fig:bestregression}):
...
\end{center}
\caption{Diagnostic plots of the best regression model. 
...
Plot consistency of data series (Figure \ref{fig:consistencyplot}):
...
@
L-moment ratios plot (Figure \ref{fig:Lmomentsratio}):
...
\end{center}
\caption{L-moment ratios plot.}
\label{fig:Lmomentsratio}
\end{figure}
...
Plot of clusters (Figure \ref{fig:4clusters}):
...
\end{center}
\caption{L-moments ratio diagram.}
\label{fig:Lmomentsratiodiagram}
\end{figure}
...
bitmap(file="GrowthCurves1.png", type="png256", height=10, width=10, res=144, pointsize=12)
@
<<fig=FALSE>>=
op <- par(mfrow=c(2,2))
 for (i in 1:nclusters) {
...
\end{center}
\caption{GrowthCurves1.png.}
\label{fig:GrowthCurves1}
\end{figure}


Regional growth-curves:
<<label=GrowthCurves2.png, echo=FALSE, eval=TRUE>>=
bitmap(file="GrowthCurves2.png", type="png256", height=10, width=10, res=144, pointsize=12)
@
<<fig=FALSE>>=
op <- par(mfrow=c(2,2))
 for (i in 1:nclusters) {
...
\end{center}
\caption{comparGrowthCurves.png.}
\label{fig:comparGrowthCurves}
\end{figure}
