...
Calberto      WRITE(KOUT,6010)I,LEN(I),NAMES(I),(XMOM(K,I),K=2,4),
Calberto     *  D(I),LOOK1,LOOK2
  150 CONTINUE
Calberto      WRITE(KOUT,6020)(RMOM(K),K=2,4)
Calberto      IF(KSTART.LT.NSITES)WRITE(KOUT,6030)(WORK(K,1),K=KSTART,NSITES)
  160 CONTINUE
C
      IF(NSIM.LE.0)RETURN
...
C
      ...
      RPARA(4)=-ONE
Calberto  180 IF(KPRINT.GT.0)WRITE(KOUT,6040)(RPARA(K),K=1,4)
  180 CONTINUE
C
...
C
Calberto      IF(KPRINT.GT.0)WRITE(KOUT,6050)NSIM
      DO 230 J=1,3
      VBAR(J)=VBAR(J)/NSIM
...
Calberto      IF(J.EQ.1)WRITE(KOUT,6060)VOBS(J),VBAR(J),VSD(J),H(J),LOOK1,LOOK2
Calberto      IF(J.EQ.2)WRITE(KOUT,6070)VOBS(J),VBAR(J),VSD(J),H(J),LOOK1,LOOK2
Calberto      IF(J.EQ.3)WRITE(KOUT,6080)VOBS(J),VBAR(J),VSD(J),H(J),LOOK1,LOOK2
  230 CONTINUE
  235 CONTINUE
C
...
Calberto      WRITE(KOUT,6090)NSIM
      DO 250 IDIST=1,5
      LOOK1=BLANK
      IF(DABS(Z(IDIST)).LT.ZCRIT)LOOK1=STAR
Calberto  250 WRITE(KOUT,6100)DISTRI(IDIST),T4FIT(IDIST),Z(IDIST),LOOK1
  250 CONTINUE
  260 CONTINUE
C
C         FIT DISTRIBUTIONS
...
Calberto      IF(NSIM.EQ.1)WRITE(KOUT,6110)
Calberto      IF(NSIM.GT.1)WRITE(KOUT,6120)
      DO 280 IDIST=1,5
Calberto      IF(DABS(Z(IDIST)).LE.ZCRIT)
Calberto     *  WRITE(KOUT,6130)DISTRI(IDIST),(PARA(IPARA,IDIST),IPARA=1,3)
  280 CONTINUE
Calberto      WRITE(KOUT,6130)DISTRI(6),(PARA(IPARA,6),IPARA=1,5)
C
C         ... AND ESTIMATE AND PRINT QUANTILES
C
      IF(NPROB.EQ.0)GOTO 320
Calberto      WRITE(KOUT,6140)PROB
      DO 300 IDIST=1,5
  ...
  290 CONTINUE
Calberto      WRITE(KOUT,6150)DISTRI(IDIST),(Q(IQ),IQ=1,NPROB)
  300 CONTINUE
      DO 310 IQ=1,NPROB
  310 Q(IQ)=QUAWAK(PROB(IQ),PARA(1,6))
Calberto      WRITE(KOUT,6150)DISTRI(6),(Q(IQ),IQ=1,NPROB)
  320 CONTINUE
C
      RETURN
C
Calberto 1000 WRITE(KOUT,7000)'MAXNS'
 1000 RETURN
Calberto 1010 WRITE(KOUT,7000)'MAXQ'
 1010 RETURN
Calberto 1020 WRITE(KOUT,7000)'MAXREC'
 1020 RETURN
Calberto 1030 WRITE(KOUT,7010)
 1030 GOTO 140
C
